...
dat.outlier.removed <- readRDS(file=here('05_CCLE_data_preparation/output/03_3_DOC_Preprocessing_removeOutliers', 'dat.outlier.removed.rds'))

col.means <- sapply(dat.outlier.removed[!grepl('chrX|chrY', rownames(dat.outlier.removed)),], mean) %>%
  as.data.frame() %>%
  setNames('mean')
...
saveRDS(dat.log2, file=here('05_CCLE_data_preparation/output/03_4_DOC_Preprocessing_log2Transformation', 'dat.log2.rds'), compress=FALSE)

...
remove.outliers <- function(list) {
  list.new <- list %>%
    as.data.frame() %>%
    setNames('signal') %>%
    bind_cols(pos.dat.flt %>% select(locus, Chr, arm)) %>%
    group_by(Chr, arm)
  ...
  for (i in 1:trailingN) {
    ...
    list.new <- list.new %>%
      ...
      select(locus, Chr, arm, signal, starts_with('last'), starts_with('next'))
  }
...
dat.outlier.removed.list <- lapply(dat.flt, remove.outliers)

# dir <- '/xchip/beroukhimlab/kei/project/Tangent/20230424_TCGAseg'
dir <- '/xchip/beroukhimlab/kei/project/TangentXY_scripts/01_TCGA_chrX_seg_arm_level_average'
setwd(dir)

library(tidyverse)
library(here)

sample.annot <- read.delim(here('data', 'clinical.tsv'))
...
seg.files <- list.files(here('data/seg'), , pattern='v2\\.txt$', recursive=TRUE, full.names=TRUE)
seg.list <- lapply(seg.files, read_delim)

seg.df <- seg.list %>%
  bind_rows() %>%
  as.data.frame()
# saveRDS(seg.df, here('tmp', 'seg.df.rds'), compress=FALSE)
# seg.df <- readRDS(file=here('tmp', 'seg.df.rds'))
